...
<form id="add_wishlist_form" class="form" action="#" enctype="multipart/form-data">
...
{{--                       
                        <div class="fv-row mb-7">
                            ...
                            <input type="text" name="title" class="form-control form-control-solid mb-3 mb-lg-0"
                                placeholder="Tax Name" value="{{ $info->customer->first_name ?? '' }}" />
                        </div> --}}
...
<script>
    var add_url = "{{ route('wishlist.save') }}";
    $('.float-number').keypress(function(event) {
        if ((event.which != 46 || $(this).val().indexOf('.') != -1) && (event.which < 48 || event.which > 57)) {
            event.preventDefault();
        }
    });
    var KTUsersWishlist = function() {
        // Shared variables
        const element = document.getElementById('kt_common_add_form');
        const form = element.querySelector('#add_wishlist_form');
        ...
        var initWishlist = () => {
            ...
            const submitButton = element.querySelector('[data-kt-wishlist-modal-action="submit"]');
